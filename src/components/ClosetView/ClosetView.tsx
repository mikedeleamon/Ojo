  ...
  <div className={styles.articleCard}>
    ...
    <div className={styles.articleInfo}>
      ...
      {article.clothingCategory && (
        <span className={styles.categoryTag}>{article.clothingCategory}</span>
      )}
...
const Chip = ({ label, active, color, onClick }: {
  ...
  <button
    ...
    onClick={onClick}
  >
    {color && <span className={styles.chipColor} style={{ background: color }} />}
    {label}
  </button>
);
...
const ClosetView = ({
  ...
  const clearFilters = () => {
    setActiveCategories([]); setActiveColors([]); setActiveFabrics([]); setQuery('');
  };
  ...
  const filteredArticles = useMemo((): ClothingArticle[] => {
    ...
    if (activeCategories.length > 0)
      arts = arts.filter(a => a.clothingCategory && activeCategories.includes(a.clothingCategory));
      ...
      <aside className={styles.sidebar}>
        ...
        {creating && (
          <div className={styles.inlineForm}>
            <input autoFocus className={styles.inlineInput} placeholder="Closet name…" value={newName}
        ...
        <nav className={styles.closetList}>
          {closets.map(c => (
            <div key={c._id}
              ...
              onClick={() => { setSelectedId(c._id); setEditingId(null); clearFilters(); }}
            >
              {editingId === c._id ? (
                <div className={styles.inlineForm} onClick={e => e.stopPropagation()}>
                  <input autoFocus className={styles.inlineInput} value={editName}
                ...
                <>
                  ...
                  <div className={styles.closetActions} onClick={e => e.stopPropagation()}>
                    <button
                      ...
                      onClick={() => onSetPreferred(c._id)} aria-label={c.isPreferred ? 'Preferred' : 'Set preferred'}
                    >
      ...
      <main className={styles.main}>
        ...
        <div className={styles.searchBar}>
          <div className={styles.searchInputWrap}>
            <svg width="15" height="15" viewBox="0 0 18 18" fill="none" className={styles.searchIcon}>
          ...
          <div className={styles.filterPanel}>
            <div className={styles.filterGroup}>
              ...
              <div className={styles.chipRow}>
                {CATEGORIES.map(c => (
                  <Chip key={c} label={c} active={activeCategories.includes(c)}
                    onClick={() => toggle(activeCategories, setActiveCategories, c)} />
                ))}
              </div>
            </div>
            <div className={styles.filterGroup}>
              ...
              <div className={styles.chipRow}>
                {COLORS.map(c => (
                  <Chip key={c} label={c} active={activeColors.includes(c)}
                    color={CSS_COLORS[c]}
            ...
            <div className={styles.filterGroup}>
              ...
              <div className={styles.chipRow}>
                {FABRICS.map(f => (
                  <Chip key={f} label={f} active={activeFabrics.includes(f)}
                    onClick={() => toggle(activeFabrics, setActiveFabrics, f)} />
                ))}
              </div>
            </div>
            {hasFilters && (
        ...
        {(query || hasFilters) && selected?.articles.length > 0 && (
          <p className={styles.resultsCount}>
            {filteredArticles.length} of {selected.articles.length} articles
          </p>
        )}
        ...
        {selected?.articles.length === 0 ? (
          ...
          <div className={styles.emptyArticles}>
            ...
            <button className={styles.clearFiltersBtn} onClick={clearFilters}>Clear search &amp; filters</button>
          </div>
        ) : (
