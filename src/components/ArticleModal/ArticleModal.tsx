...
import { PhotoLibraryIcon } from '../icons/GlyphIcons';
...
const EMPTY: ArticleFormData = {
    ...
    imageUrl: '',
    detectedGarmentType: undefined,
    detectedColors: undefined,
    detectedFabric: undefined,
...
const MultiSwatch = () => (
    ...
    >
        <Path
            d='M14,14 L14,0 A14,14 0 0,1 28,14 Z'
            fill='#ef4444'
        />
        <Path
            d='M14,14 L28,14 A14,14 0 0,1 14,28 Z'
            fill='#3b82f6'
        />
        <Path
            d='M14,14 L14,28 A14,14 0 0,1 0,14 Z'
            fill='#22c55e'
        />
        <Path
            d='M14,14 L0,14 A14,14 0 0,1 14,0 Z'
            fill='#fbbf24'
        />
    </Svg>
);
...
const TypePickerField = ({
    ...
    return (
        <View style={st.field}>
            <FieldLabel>
                Type <Text style={st.required}>*</Text>
            </FieldLabel>
                        ...
                        <View style={st.chipGrid}>
                            {group.types.map((type) => (
                                <Pressable
                                    ...
                                    style={[
                                        st.chip,
                                        value === type && st.chipActive,
                                    ]}
                                    onPress={() => {
                                        if (value !== type) onChange(type);
                                    }}
            ...
            <View style={st.swatchGrid}>
                {COLORS.map((c) => (
                    <Pressable
                        ...
                        style={[
                            st.swatchRing,
                            value === c && st.swatchRingActive,
                        ]}
                        hitSlop={{ top: 4, bottom: 4, left: 4, right: 4 }}
                    >
                        {c === 'Multi' ? (
                            <View style={st.swatch}>
                                <MultiSwatch />
                            </View>
...
const ArticleModal = ({
    ...
    const [form, setForm] = useState<ArticleFormData>(
        initialData ? toForm(initialData) : EMPTY,
    );
        ...
        uploadImageToR2(uri, closetId)
            .then((r2Url) => {
                ...
                if (r2Url) {
                    ...
                    Alert.alert(
                        'Upload failed',
                        'Could not upload the image. You can add one manually.',
                    );
                }
            })
    ...
    const set = <K extends keyof ArticleFormData>(
        key: K,
        val: ArticleFormData[K],
    ) => {
    ...
    const runIdentification = async (
        ...
        try {
            ...
            const result = await identifyClothing(cropped.uri, {
    ...
    const handleTypeChange = (type: string) => {
        ...
        setForm((f) => {
            ...
            return {
                ...
                fabricType: fabricIsAutoValue
                    ? (newDefaults.fabricType ?? '')
                    : f.fabricType,
                gender: genderIsAutoValue
                    ? (newDefaults.gender ?? 'Unisex')
                    : f.gender,
            ...
            >
                ...
                >
                    ...
                    <View style={st.imageSection}>
                        ...
                        {/* Detection results */}
                        {identifying ||
                        form.detectedGarmentType != null ||
                        (form.detectedColors &&
                            form.detectedColors.length > 0) ? (
                            <View style={st.detectionCard}>
                                {identifying ? (
                                    <Text style={st.detectionLabel}>
                                        🔍 Identifying…
                                    </Text>
                                ) : (
                                    <>
                                        ...
                                        {(() => {
                                            const colorName = form
                                                .detectedColors?.[0]
                                                ? (DETECTED_COLOR_TO_FORM[
                                                      form.detectedColors[0]
                                                          .name
                                                  ] ??
                                                  form.detectedColors[0].name)
                                                : '';
                                            // Prefer the mapped form-type name; fall back to raw ML Kit label text
                                            const garmentName =
                                                form.detectedGarmentType &&
                                                form.detectedGarmentType !==
                                                    'unknown'
                                                    ? (GARMENT_TO_FORM_TYPE[
                                                          form
                                        ...
                                        <View style={st.detectionRow}>
                                            {form.detectedFabric &&
                                                form.detectedFabric.type !==
                                                    'unknown' && (
                                                    <View
                                                        style={st.detectionChip}
                                                    >
                                                        <Text
                                                            style={
                                                                st.detectionChipText
                                                            }
                                                        >
                                                            {
                                                                form
                                            ...
                                            {form.identificationConfidence !=
                                                ...
                                                form.identificationConfidence >
                                                    ...
                                                    <View
                                                        style={[
                                                            st.detectionChip,
                                                            st.detectionChipMuted,
                                                        ]}
                                                    >
                                                        <Text
                                                            style={
                                                                st.detectionChipTextMuted
                                                            }
                                                        >
                                                            {Math.round(
                                                                form.identificationConfidence *
                                        ...
                                        {form.detectedColors &&
                                            form.detectedColors.length > 0 && (
                                                <View
                                                    style={st.detectionColors}
                                                >
                                                    {form.detectedColors.map(
                                                        (c, i) => (
                                                            <View
                                                                key={i}
                                                                style={
                                                                    st.detectionSwatchWrap
                                                                }
                                                            >
                                                                <View
                                                                    style={[
                    ...
                    <View style={st.field}>
                        <FieldLabel>
                            Name <Text style={st.optional}>(optional)</Text>
                        </FieldLabel>
                        <TextInput
                            style={st.input}
                    ...
                    <View style={st.field}>
                        <FieldLabel>
                            Purchase Price{' '}
                            <Text style={st.optional}>(optional)</Text>
                        </FieldLabel>
                        <TextInput
                            style={st.input}
                        ...
                        {form.purchasePrice == null && (
                            ...
                            >
                                💡 Add a price to unlock cost-per-wear in
                                Insights.
                            </Text>
                        )}
                    </View>
              ...
              All named accessories set isAccessory automatically from the type. */}
                        ...
                        <View style={st.field}>
                            ...
                            <View style={st.chipGrid}>
                                <Pressable
                                    style={[
                                        st.chip,
                                        form.isAccessory && st.chipActive,
                                    ]}
                                    onPress={() =>
                                        setForm((f) => ({
                                            ...f,
                                            isAccessory: !f.isAccessory,
                                            bodyZone: f.isAccessory
                                                ? undefined
                                                : f.bodyZone,
                                        }))
                                    }
                    ...
                    {form.isAccessory && (
                        <ChipField
                            ...
                            items={BODY_ZONES}
                            onValueChange={(v) =>
                                set('bodyZone', (v as BodyZone) || undefined)
                            }
                        />
                    )}
