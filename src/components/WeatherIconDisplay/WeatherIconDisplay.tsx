...
const iconFor = (condition: string, isDay: boolean) => {
  ...
  if (c.includes('rain')    || c.includes('shower') || c.includes('drizzle'))  return ICONS.Rainy;
  if (c.includes('sunny')   || c.includes('mostly sunny'))                     return ICONS.Sunny;
  if (c.includes('clear')   || c.includes('mostly clear'))                     return isDay ? ICONS.Sunny : ICONS.ClearNight;
  if (c.includes('partly')  || c.includes('intermittent') || c.includes('hazy'))
    return isDay ? ICONS.PartlyCloudy : ICONS.PartlyCloudyNight;
  if (c.includes('cloud')   || c.includes('overcast') || c.includes('dreary')) return ICONS.Cloudy;
...
const styles = StyleSheet.create({
  ...
  large:     { gap: 8 },
  small:     { gap: 4 },
  iconLarge: { width: 120, height: 120 },
  iconSmall: { width: 36,  height: 36  },
  temps:     { alignItems: 'center', gap: 2 },
  temp:      { fontFamily: fonts.display, fontSize: 64, color: colors.textPrimary, lineHeight: 68 },
  feelsLike: { fontFamily: fonts.body,    fontSize: fontSizes.sm, color: colors.textSecondary },
  miniTemp:  { fontFamily: fonts.body,    fontSize: fontSizes.sm, color: colors.textSecondary },
});
