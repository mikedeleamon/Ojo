...
import { auth, AUTH_KEY, getToken } from '../../lib/auth';
...
const PreferencesTab = ({ settings, saveSettings }: {
  settings: Settings;
  saveSettings: (s: Settings) => Promise<void>;
}) => {
  ...
  const { status, loading, submit } = useFormSubmit('Preferences saved.', 2500);

  const save = () => submit(() =>
    saveSettings({ clothingStyle, location, temperatureScale: tempScale,
  ...
  return (
    <div className={styles.tabContent}>
        ...
        <div className={styles.chips}>
          {STYLES.map(s => (
            <button key={s}
              ...
              onClick={() => setClothingStyle(s)}>{s}</button>
          ))}
        </div>
      </section>
      ...
      <section className={styles.section}>
        ...
        <input className={styles.input} type="text" placeholder="City name"
      ...
      <section className={styles.section}>
        ...
        <div className={styles.segmented}>
          <button className={`${styles.seg} ${tempScale === 'Imperial' ? styles.segActive : ''}`}
            onClick={() => setTempScale('Imperial')}>°F</button>
          <button className={`${styles.seg} ${tempScale === 'Metric'   ? styles.segActive : ''}`}
            onClick={() => setTempScale('Metric')}>°C</button>
        </div>
      </section>
      ...
      <StatusMessage status={status} styles={styles} />
      <button className={styles.saveBtn} onClick={save} disabled={loading}>Save changes</button>
    </div>
  );
};
    ...
    <div className={styles.tabContent}>
      ...
      {entries.length === 0 ? (
        ...
        <div className={styles.historyList}>
          {entries.map((entry, i) => (
            <div key={entry.id} className={styles.historyCard} style={{ animationDelay: `${i * 0.04}s` }}>
              ...
              <button className={styles.historyDeleteBtn} onClick={() => handleDelete(entry.id)} aria-label="Remove entry">
...
const AccountPage = ({ settings, saveSettings, onLogout }: Props) => {
  ...
  return (
    <div className={styles.root}>
      ...
      {showDeleteModal && (
        <div className={styles.modalOverlay}
          onClick={e => { if (e.target === e.currentTarget) { setShowDeleteModal(false); setDeleteError(null); } }}>
          <div className={styles.deleteModal}>
            ...
            <div className={styles.deleteModalActions}>
              <button className={styles.deleteModalCancel}
                onClick={() => { setShowDeleteModal(false); setDeleteError(null); }}
                disabled={deleteLoading}>Cancel</button>
              <button className={styles.deleteModalConfirm}
