...
const AppRoutes = ({
  ...
  return (
    <Routes>
      ...
      <Route path="/settings" element={guard(<SettingsPage settings={settings} saveSettings={saveSettings} />)} />
      <Route path="/closet"   element={guard(<ClosetPage />)} />

      {/* ── Account / Settings flat-list architecture ── */}
      <Route path="/account" element={guard(<SettingsScreen onLogout={onLogout} />)} />

      {/* Detail screens — each 1 level deep, matching SETTINGS_CONFIG routes */}
      <Route path="/account/profile"        element={guard(<ProfileScreen     onLogout={onLogout} />)} />
      <Route path="/account/password"       element={guard(<PasswordScreen    />)} />
      <Route path="/account/history"        element={guard(<HistoryScreen     />)} />
      <Route path="/account/preferences"    element={guard(<PreferencesScreen />)} />
      <Route path="/account/notifications"  element={guard(<NotificationsScreen />)} />
      <Route path="/account/permissions"    element={guard(<PermissionsScreen />)} />
      <Route path="/account/data-usage"     element={guard(<DataUsageScreen   />)} />
