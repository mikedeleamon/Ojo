...
const userSchema = new Schema<IUser>({
  ...
  // No date of birth, deliberately. Ojo required one until App Review rejected
  // build 31 under guideline 5.1.1(v); the `birthday` and `ageVerifiedAt`
  // values older accounts still hold are removed by
  // scripts/purgeBirthdays.ts.
